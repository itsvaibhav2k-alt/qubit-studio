...
(async () => {
  ...
  try {
    ...
    const myla = page.locator('.myla-pop[role="dialog"]');
    await myla.waitFor();
    ...
    assert.equal(await myla.locator('.ask-answer').count(), 2, 'Multiple selected topics retain their local notes');
    await myla.getByRole('button', { name: 'Clear topic selection', exact: true }).click();
    assert.ok(await myla.getByRole('button', { name: 'Ask Gemini', exact: true }).isDisabled());
    await myla.getByRole('checkbox').first().check();
    await myla.locator('summary', { hasText: 'Choose explanation topics' }).click();
    await myla.getByRole('button', { name: 'Ask Gemini', exact: true }).click();
