  ...
  return (
    <div className="drive-cartoon" data-tour="drive-cartoon">
      ...
      <p className={`drive-note${leaky ? ' warn' : ''}`}>
        {leaky
          ? <MathText text={`$|\\alpha|$ is only ${num(alpha, 0)} MHz — a short pulse is spectrally wide enough to nibble $|2\\rangle$.`} />
          : <MathText text={`$|\\alpha| = ${num(alpha, 0)}$ MHz of detuning. A typical few-hundred-ns pulse sits well below $f_{12}$.`} />}
      </p>
    </div>
  );
}
