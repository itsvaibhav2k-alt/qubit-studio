...
export default function LayoutArtwork({ selected, hiddenParts = [], componentMaterials = DEFAULT_COMPONENT_MATERIALS, onSelect, onInspect, detail, miniature, pixelsPerUnit = 1, layerColors = false }: Props) {
  ...
  return <g className={`layout-artwork${layerColors?' uses-layer-colors':''}`}>
    ...
    {visible('board') && <g {...pick('board','Carrier board — chip support')} clipPath={fill('region')}>
      ...
      <rect x={500-.445/DIE_SCALE*world} y={310-.445/DIE_SCALE*world} width={.89/DIE_SCALE*world} height={.89/DIE_SCALE*world} rx="2" fill={fill('seat')}/>
    </g>}
    {visible('package') && <g {...pick('package','Package aperture — surrounding shield detail')}>
      <path className="part-outline" d={apertureContour} fill={fill('package')} fillRule="evenodd" stroke={surface('package').edge} strokeWidth=".7"/>
      <path d={apertureContour} fill={finish('package')} fillRule="evenodd"/>
      ...
      <rect x={500-aperture/2-2} y={310-aperture/2-2} width={aperture+4} height={aperture+4} rx="13" fill="none" stroke={surface('package').dark} strokeWidth="4"/>
      {sides.map(side=><g key={side} transform={`rotate(${side*90} 500 310)`} fill={fill('contact')}>
        {Array.from({length:BOND_COUNT},(_,i)=><rect key={i} x={500+bondPosition(i)*1.05*world-.009/DIE_SCALE*world/2} y={310+(PLATE.window/2+.040)/DIE_SCALE*world-.043/DIE_SCALE*world/2} width={.009/DIE_SCALE*world} height={.043/DIE_SCALE*world} rx=".65" stroke="#f3d899" strokeWidth=".25"/>)}
      </g>)}
      ...
      {sides.map(side=><g key={side} aria-hidden="true" transform={`rotate(${side*90} 500 310)`} fontFamily="monospace" fontSize="4.1" fill={surface('package').dark}>
      ...
      <g aria-hidden="true" fill="#303a36" fontFamily="monospace" fontSize="5.5" letterSpacing=".6"><text x="500" y="63" textAnchor="middle">QS–01 / SHIELD APERTURE</text><text x="500" y="562" textAnchor="middle">TRANSMON · REV A</text></g>
    </g>}
    ...
    {visible('ground') && <g {...pick('ground','Ground metal — openings expose the substrate')}>
      ...
      <g clipPath={fill('substrate-clip')} data-detail="film-fanouts" fill={layerColors?'#b8cbb4':surface('ground').edge} stroke={layerColors?'#b8cbb4':surface('ground').edge}>
        <g transform={FILM_TRANSFORM}>
          {sides.map(side=><g key={side} transform={`rotate(${side*90})`}>
            {FILM_LAUNCHES.map((launch,i)=><g key={i}><rect x={launch.x-4.5} y="936" width="9" height="38" stroke="none" rx="1.2"/><rect x={launch.x-2.3} y="941" width="4.6" height="26" fill={surface('ground').dark} stroke="none"/><rect x={launch.x-3} y="904" width="6" height="14" stroke="none"/><polyline points={polygon(launch.points)} fill="none" strokeWidth={launch.width}/></g>)}
      ...
      <g data-detail="bond-contacts" fill={fill('contact')}>
        {sides.map(side=><g key={side} transform={`rotate(${-side*90} 500 310)`}>
          {Array.from({length:BOND_COUNT},(_,i)=>{const points=bondPoints(i,side).map(([x,,z])=>inspectionWorldPoint(x,z));return <g key={i}><polyline points={polygon(points)} fill="none" stroke="#312918" strokeOpacity=".55" strokeWidth={.0045*world} strokeLinejoin="round"/><polyline points={polygon(points)} fill="none" stroke={BOND_FINISH.wire} strokeWidth={.0023*world} strokeLinejoin="round"/><polyline points={polygon(points)} fill="none" stroke="#f8e3b4" strokeWidth={.0006*world} strokeLinejoin="round"/>{[points[0],points[2]].map(([x,y],j)=><ellipse key={j} cx={x} cy={y} rx={.0032*world} ry={.0062*world} stroke="#f4d796" strokeWidth=".23"/>)}</g>;})}
        </g>)}
      </g>
    </g>}
    {visible('capacitor') && <g transform={PLAN_TRANSFORM}>
      {[['left',LEFT_ELECTRODE],['right',RIGHT_ELECTRODE]].map(([side,path])=><g key={side} {...pick('capacitor',`${side==='left'?'Left':'Right'} capacitor pad — charging energy`)} data-side={side}>
        <path className="part-outline" d={path} fill={fill('capacitor')} stroke={surface('capacitor').edge} strokeWidth=".8" vectorEffect="non-scaling-stroke"/>
    ...
    {visible('junction') && <g {...pick('junction','Josephson junction — Josephson energy')} data-detail="junction-overlap">
      ...
      <rect x={500-JUNCTION_OVERLAP.width*world/2} y={310+(JUNCTION_OVERLAP.z-JUNCTION_OVERLAP.depth/2)*world} width={JUNCTION_OVERLAP.width*world} height={JUNCTION_OVERLAP.depth*world} fill={JUNCTION_OVERLAP.color} stroke="#e2d5ff" strokeWidth=".35"/>
    ...
    {!miniature && <g data-detail="layout-identifiers" aria-hidden="true" pointerEvents="none" fontFamily="monospace">
      {visible('capacitor')&&<g fill="#364b57" fontSize={fine?6:5.4} letterSpacing=".65">
      ...
      {visible('ground')&&<g fill="#d3dfdf" fontSize="4.7" letterSpacing=".65"><text x="500" y="189" textAnchor="middle">GND / {material('ground').formula} FILM</text><text x="500" y="454" textAnchor="middle">PERFORATED GROUND METAL</text></g>}
      {fine&&visible('ground')&&sides.map(side=><g key={side} transform={`rotate(${side*90} 500 310)`} fill="#c3cbd1" fontSize="3.1">{Array.from({length:12},(_,i)=><text key={i} x={500+bondPosition(i*4)*world} y="492" textAnchor="middle">B{String(i*4+1).padStart(2,'0')}</text>)}</g>)}
    </g>}
  </g>;
}
