...
export function composeLocalMyla(topic: TopicId, snapshot: ChipSnapshot): ExplainResult {
  const p = snapshot.params;
  const o = snapshot.outputs;
  const title = formatTopicHeadline(topic, snapshot);
  const ctx = snapshot.context;
  const ratio = p.ratio;
  const body = brief(topic, snapshot, p, o, ratio, ctx);
  return { topic, title, body, model: 'gemini-2.5-flash' };
}
...
function brief(
  ...
  switch (topic) {
    ...
    case 'materials':
      return `Live stack is ${ctx?.top_material ?? 'the top metal'} on ${ctx?.base_material ?? 'the base'}. Color is a room-light stand-in so you can tell films apart — not a measured kinetic inductance.\nThis isolated-transmon solver still takes $E_J$ and $E_C$ as numbers. It does not compute $T_1$ from the catalog.`;
    case 'goals':
      return `Targets are $f_{01}=${num(ctx?.target_ghz ?? 5, 2)}\\,\\mathrm{GHz}$, $|\\alpha|\\ge ${num(ctx?.min_anharmonicity_mhz ?? 200, 0)}\\,\\mathrm{MHz}$, dispersion $\\le ${num(ctx?.max_dispersion_khz ?? 10, 1)}\\,\\mathrm{kHz}$.\nLive chip: $f_{01}=${num(f01, 2)}\\,\\mathrm{GHz}$, $\\alpha=${signed(alpha, 0)}\\,\\mathrm{MHz}$. Search only scans a grid of $E_J,E_C$; a pass is not a fabricated optimum.`;
    case 'assembly':
      return `Explode is camera/layout only (now ${num(ctx?.explode ?? 0, 2)}). Nothing in the Hamiltonian depends on it.\nUse it to see the junction versus the pads; then collapse it before you screenshot a “chip.”`;
    case 'baseline': {
    ...
    case 'search':
      return `“Try a goal” walks a 1-D/2-D grid of $E_J$ and $E_C$ and keeps points that pass your $f_{01}$, $|\\alpha|$, and dispersion cuts.\nIt is exhaustive on that grid only — not a process window, not a full-chip EM extract.`;
    case 'stress':
      return `Robustness wiggles $E_J$ and $E_C$ by a few percent to mimic junction-area and pad-size scatter.\nWatch $f_{01}$ span and dispersion: if they blow past your goals, the design is numerically legal but too sharp to build. This is not a yield model.`;
    case 'tunable':
